# Cortag USA — Contexto de marca
Última actualización: abril 2026

## Qué es Cortag USA
Marca brasileña de herramientas profesionales para instalación cerámica/porcelánico.
Mercado: Estados Unidos exclusivamente.
Modelo: B2B EXCLUSIVO — sin e-commerce, venta vía distribuidores autorizados.
Sede operativa Europa: Granollers, Barcelona (gestión Team4Digital).
CEO: Leandro Pitzeta (Brasil).

## Contactos clave
- Juan Luis Arronte: National Sales Manager USA (contacto principal)
- Raúl: Equipo comercial USA
- Marta Servián (Team4Digital): Gestión contenido y social media

## Canal principal
Instagram: @cortagusa
Target: Professional tile installers, tile contractors, tile pros USA
Seguidores: ~2,500 aprox

## Tono y estilo — MUY IMPORTANTE
- Inglés AMERICANO (no británico, no traducción del español)
- Casual profesional, nunca corporativo
- Contracciones naturales: we're, you'll, it's, can't
- Palabras clave: awesome, check out, stop by, grab, killer, pro-level
- NUNCA: utilise, brilliant, lovely, "we are pleased to announce"
- Emojis moderados: 🔧 🔥 💪 ✅ 🎯

## Terminología técnica USA
- Large format tile (no "gran formato")
- GPTP (Gauged Porcelain Tile Panels)
- Tile pro / Tile installer / Tile contractor
- Cut and snap (no "corte y separación")
- Score and snap
- Thinset / Mortar (no "adhesivo")
- Grout lines / Lippage / Flatness
- Medidas en imperial SIEMPRE (con métrico entre paréntesis)
  Ejemplo: "cuts up to 134\" (3400mm)"

## Hashtags sector USA
#TileInstallation #PorcelainTile #Cortag #TileTools
#ProfessionalInstaller #LargeFormatTile #TilePro
#CeramicTile #CortagUSA #TileContractor #GPTP

## Estructura contenido
- Hook fuerte en las primeras 3 palabras
- Beneficio claro del producto
- CTA específico cuando aplica
- Hashtags al final

## Presupuesto Meta Ads
$50/mes — EXTREMADAMENTE limitado, guardado para acciones grandes.
Sin inversión en ads para sorteos o amplificación de contenido habitual.

## Situación contenido
- Sin equipo de grabación propio en USA
- Dependencia total de UGC orgánico + embajadores
- Equipo comercial envía fotos/vídeos estáticos ocasionalmente
- NO hay contenido con presentación a cámara propia

## Embajadores activos Q2 2026
- Dumitru Guleac (@dg_pro_remodeling_llc): MANTENER ✅
  14,700 seguidores, mejor performer, cumple compromisos
- Missy McMaster (TrueLine Tile and Stone): NUEVA 🆕
  Oregon, contrato firmado, selección productos pendiente
- Phillip Harms (@versatile_llc): CONDICIONAL 🟡
  1,261 seguidores, muy proactivo pero calidad baja
- Daniel Becerra (@yourtiledesignguy): DISCONTINUAR ❌
  Ghosting total, incumplimiento

## Modelo embajadores
Producto gratuito por contenido (sin compensación económica).
Limitación: compromiso irregular sin incentivo económico.
Propuesta Q2: piloto creador pagado $300-500/mes para resolver gap.

## Ferias y eventos
- Coverings (principal evento sector USA, Las Vegas) — anual
- Formación Neolith GPTP (Pembroke Park, Florida, junio 2026)

## Competidores y conflictos de marca
- **Competidores directos (evitar o verificar exclusividad antes de contactar):** Rubi, Grabotools, y marcas similares de herramientas de corte de cerámica
- **Marcas complementarias (sin conflicto):** Schluter Systems — sus productos (perfiles, sistemas de nivelación, membranas) son complementarios a las herramientas Cortag, no competidores. Un creador con vínculos con Schluter no es un problema en sí mismo.
- En outreach a influencers: verificar si trabajan o tienen contrato de exclusividad con Rubi, Grabotools u otras marcas de herramientas de corte antes de contactar.

## Diferencias clave vs Cortag Europe
- Sin e-commerce (B2B puro via distribuidores)
- Presupuesto ads muy limitado ($50/mes vs ~400€ Europe)
- Sin Cortag Expert ni newsletter activa
- Embajadores gratis vs Rubén Baz pagado en Europe
- Contenido 100% inglés americano